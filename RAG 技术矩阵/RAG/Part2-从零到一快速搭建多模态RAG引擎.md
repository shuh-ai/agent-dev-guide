...
> 本文是 AI 大模型技术社区「RAG 检索增强生成」系列的第 2 篇，带你用 Unstructured + PaddleOCR + LangGraph + FAISS 从零搭建一个支持 PDF 图文混排检索的 Agentic RAG 系统。
>
...
```
┌─────────────────────────┐    ┌──────────────────┐    ┌──────────────────────┐
│  阶段一：文档解析         │    │  阶段二：知识库构建  │    │  阶段三：Agentic RAG  │
│                          │    │                    │    │                      │
│  PDF 文档                 │    │  Markdown 文档      │    │  用户提问              │
│     │                    │    │     │              │    │     │                │
│     ▼                    │    │     ▼              │    │     ▼                │
│  Unstructured            │    │  MarkdownHeader    │    │  Agent 决策           │
│  + PaddleOCR             │    │  TextSplitter      │    │  ├─ 是否需要检索？     │
│     │                    │    │     │              │    │  ├─ 调用 retriever    │
│     ▼                    │    │     ▼              │    │  ├─ 评估检索结果       │
│  结构化 Markdown          │    │  FAISS 向量库       │    │  ├─ 必要时重写问题     │
│  + 图片文件夹              │    │                    │    │  └─ 生成最终回答       │
└─────────────────────────┘    └──────────────────┘    └──────────────────────┘
```

![多模态文档处理管线](https://developer-blogs.nvidia.com/wp-content/uploads/2026/02/image1.png)

> ▲ 企业级多模态文档处理管线：PDF 摄入 → 多模态 Embedding → 重排序 → LLM 生成（来源：NVIDIA Technical Blog）
...
![Unstructured 文档解析流程](https://miro.medium.com/v2/resize:fit:1358/format:webp/1*5_LKV1bPAad6dSyiLKz7eg.png)

> ▲ Unstructured 库的文档分区（Partitioning）流程：多格式文档 → 元素提取 → 结构化输出（来源：Medium）
...
![FAISS 向量检索流程](https://miro.medium.com/v2/resize:fit:1358/0*qyTP9OJW0HVWA2KX)

> ▲ FAISS 向量语义检索原理：Query 和 Documents 分别 Embedding → 向量空间中计算邻近度 → 返回最近邻（来源：Medium）
...
└──────┬──────────────┘
       ...
       ▼
┌─────────────────────┐
│  Retriever 检索      │ ← FAISS 语义搜索 (Top-3)
└──────┬──────────────┘
       │
       ▼
┌─────────────────────┐     ┌─────────────────────┐
│  Grader 评估相关性    │─no─→│  Rewriter 改写问题   │──→ 回到第一步
└──────┬──────────────┘     └─────────────────────┘
       │ yes
       ▼
┌─────────────────────┐
│  LLM 生成回答        │ ← 基于检索上下文 + Markdown 格式
└─────────────────────┘
```

这个设计对应 Part 1 中介绍的 **Self-RAG** 思想——系统会自我评估检索质量，不满足时自动改写问题重试。

![多模态 RAG 检索管线](https://assets.zilliz.com/Fig_2_Multimodal_RAG_retrieval_pipeline_e5d3e8f393.png)

> ▲ 多模态 RAG 检索管线：文本+图片并行 Embedding → 多路向量检索 → 融合排序 → LLM 生成（来源：Zilliz）
...
- **Part 3**：RAG 系统评估与优化（RAGAS 评估 + Reranker 实战）
- **Part 4**：GraphRAG 从入门到实战
- **Part 5**：企业级 RAG 部署（Docker + API 服务化）

> 📢 更多讨论、源码、资料，欢迎加入 AI 大模型技术社区，与数千名开发者一起交流学习。
