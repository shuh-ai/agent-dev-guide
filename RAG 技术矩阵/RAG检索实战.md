...
> 核心要点：** 在 RAG 系统中，开发者常常面临一个两难选择：切片（Chunk）切小了，检索比较精准，但丢给大模型的上下文支离破碎；切片切大了，上下文虽然完整，但包含大量无关噪声，检索准确率下降。这就像你做饭时切菜——切太小了容易糊，切太大了又不入味，火候掌握是门艺术。
...
> 普通RAG的回答就像你问路时对方说"往那边走就到了"——请问"那边"是哪边？Small-to-Big RAG则是直接给你画了张地图。
...
  然而，在企业级应用中，纯向量检索存在一个明显的**"语义盲区"**：它对**精确匹配（Exact Match）**不够敏感。就像你用模糊搜索找人——输入‘小王’，结果出来‘小王八’，你说尴尬不尴尬。
...
> 问缩写，回答"请查阅内部文档"？这跟考试时写"详见课本第X页"有什么区别。混合RAG直接甩出ESOP，干脆利落！
...
在前两个场景中，我们分别探讨了 **Sentence Window Retrieval**（解决上下文连贯性问题）和 **Hybrid Search**（解决精确匹配问题）。但在真实的企业级应用中，还有一种更复杂的挑战：**数据多样性**。这就像你开了一家餐厅，既有中餐、西餐，还有日料和甜点——一个厨师根本忙不过来，得请不同菜系的师傅。
...
| 数据特征 | 推荐策略 |
| :--- | :--- |
| 纯文本 + 长文档 + 需要细节上下文 | 场景一 (Sentence Window) |
| 大量专有名词/缩写/版本号 | 场景二 (Hybrid Search) |
| 分散在多个不同来源中 | 场景三 (Route Retrieval) |
| 结构化表格/CSV | 场景四 (PandasQueryEngine) |
| PDF + 包含图片图表 | 场景五 (MinerU + Qwen-VL) |
| 纯图片/照片库 | 场景六 (文搜图/图搜图) |
| 视频文件 | 场景七 (Twelve Labs Video RAG) |

> 记住这个选型口诀：**文本看场景，表格用Pandas，PDF上MinerU，图片靠向量，视频找Twelve**。
