...
大语言模型（LLM）的能力边界受限于其训练数据的截止时间与静态知识结构。要让模型具备实时查询天气、操作数据库、调用第三方 API 等动态能力，就必须为其接入外部工具。

在 MCP 出现之前，每接入一个新工具或数据源，开发者都需要编写大量**胶水代码（Glue Code）**：定义调用协议、处理鉴权、解析返回值、转换数据格式。更严重的问题是，这些适配代码是**碎片化**的——为 LangChain 编写的工具函数无法直接在 Claude Desktop 或 Cursor 中使用，反之亦然。这导致了典型的 **N×N 集成困境**：
...
**MCP（Model Context Protocol，模型上下文协议）** 是由 Anthropic 于 2024 年 11 月提出并开源的开放性技术标准，旨在为大语言模型与外部工具、数据源、服务之间定义一套**通用的通信契约**。

MCP 的核心思想是：**将工具与数据源抽象为标准化的服务接口**，任何实现了 MCP Client 的应用都可以通过同一套协议去发现、调用任意 MCP Server 暴露的能力。
...
正如 USB-C 标准化了物理设备的连接方式——无论接头另一端是显示器、硬盘还是电源适配器，只要遵循同一物理标准即可实现"即插即用"——MCP 也为 AI 应用提供了标准化的**数据交换与能力发现接口**。
...
MCP 遵循严格的 C/S（Client-Server）架构，由三个层次分明的角色构成：
...
- **调用模式**：由 LLM 自主决定"是否调用"以及"传入什么参数"（模型驱动的调用）
...
- **调用模式**：一般由 Host 在用户触发时主动加载（用户驱动的调用）
- **用途**：为常见任务提供标准化入口，降低 Prompt Engineering 的重复工作
...
`FastMCP` 是 MCP Python SDK 提供的高层封装，使用装饰器风格快速定义 Server 的能力。它自动处理 JSON-RPC 序列化、能力协商和传输层管理，让开发者只需关注业务逻辑。
...
`langchain-mcp-adapters` 是 LangChain 官方提供的 MCP 适配器库，它将任意 MCP Server 暴露的工具转换为 LangChain 标准的 `BaseTool` 对象，无缝融入 LangChain 的 Agent 体系。
...
对于更精细地控制工具调用流程的场景，可以直接使用 `bind_tools` 手动编排：
...
这是 MCP 的真正威力所在——同时连接多个 Server，Agent 根据任务自动选择合适的工具：
...
MCP 协议通过一套标准化的通信契约，彻底改变了 LLM 与外部工具集成的方式：

1. **架构层面**：通过 Host → Client → Server 三层解耦，实现了工具能力的"一次开发、到处接入"
2. **开发效率**：使用 FastMCP 的装饰器风格，开发者只需关注业务逻辑，无需关心 JSON-RPC 序列化细节
3. **生态整合**：通过 `langchain-mcp-adapters`，MCP Server 的能力可以零摩擦地融入 LangChain 的 `AgentExecutor` 体系
