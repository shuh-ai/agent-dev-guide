...
一个完整的 **Agent（智能体）** 由两大核心组件构成：

- **模型（Model）**：充当 Agent 的"决策中枢"，负责语义理解、逻辑推理以及任务步骤的拆解规划
- **工具（Tool）**：充当 Agent 的"执行器官"，负责调用外部 API、执行计算、查询数据库等具体操作
...
**工具的本质是一个可被模型调用的函数**。要让模型正确使用工具，需要将函数的以下信息传递给大模型：
...
使用 `@tool` 装饰器将一个普通函数声明为 Agent 可用工具：
...
LangChain 自动完成了"推理 → 调用工具 → 观测结果 → 生成回复"的完整循环，开发者无需手动编排这些步骤。

---

## 3. 自定义工具详解

LangChain 大幅简化了工具定义流程，与编写普通函数的体验几乎一致。核心思路是：**用 `@tool` 装饰器将普通函数"升级"为工具**，框架自动处理序列化、参数解析和结果回传。

### 3.1 基本定义

在函数上添加 `@tool` 装饰器即可创建一个工具。以计算平方根为例：
...
**`@tool` 装饰器的工作原理**：它会自动从函数中提取以下信息，构建工具的元数据：
...
当需要精细控制工具的输入参数时，可借助 **Pydantic v2** 对入参进行强类型约束。Pydantic 模型中的 `Field(description=...)` 会被自动提取为参数描述，帮助大模型理解每个参数的含义。

**为什么需要 Pydantic？** 简单场景下，函数签名的类型注解已经足够。但在以下情况中，Pydantic 提供了更强大的控制能力：
...
创建 Agent 时将工具列表传入，模型便会在推理过程中"看到"可用工具，按需选择调用：
...
例如，大模型的知识截止于训练数据，无法获取实时信息。配合 **Web 搜索工具**后，Agent 便具备了联网检索能力，回答的时效性和准确性大幅提升。
...
可以看到，Agent 自动调用了 Tavily 搜索工具获取实时天气数据，然后基于搜索结果生成了结构化的天气预报回复。
...
`TavilySearch` 的默认参数较多，工具描述也较为冗长。如果只需要 `query` 参数，推荐做一层轻量封装，降低模型的理解成本：
...
通过结构化输出，AI 的回复既包含准确答案，又附带可信来源，大幅提升了内容的可验证性和实用性。
