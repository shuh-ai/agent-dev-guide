...
> **提示**：后续代码示例使用 `load_dotenv()` 加载环境变量，请确保已安装 `python-dotenv` 包（`uv add python-dotenv`）。

---

## 2. 🧩 快速创建 Agent

LangChain 提供了 `create_agent` 方法，只需**模型**和**工具**即可快速创建 Agent。该方法是 1.x 的核心 API，底层基于 LangGraph 的图式执行引擎，自动处理推理循环、工具调用和结果组装。
...
运行后，Agent 会自动识别需要调用 `get_weather` 工具，获取天气数据后组织回复。
...
**输出结构说明**：`result` 中包含完整的对话消息列表，依次为用户消息、AI 的推理消息、工具调用消息、工具返回消息、以及 AI 的最终回复。LangChain 自动完成了"推理 → 调用工具 → 观测结果 → 生成回复"的完整循环，开发者无需手动编排。

**关于 `@tool` 装饰器**：LangChain 通过 `@tool` 装饰器将普通 Python 函数转化为 Agent 可调用的工具。装饰器会自动从函数名和 docstring 中提取工具名称、参数描述等信息。docstring 写得越清晰，Agent 就越能准确判断何时调用以及如何传参——这是 Agent 开发中最值得关注的细节之一。
...
`create_agent` 的 `"provider:model"` 格式支持切换不同供应商的模型，工具定义和业务逻辑完全不变。
...
2. **模型推理（Reasoning）**：识别到"今日天气"属于实时数据，模型自身无法直接作答，判定需要调用外部工具 `get_weather`
3. **执行动作（Action）**：触发 `get_weather` 工具，传入参数 `"成都"`
4. **结果观测（Observation）**：接收工具返回的数据，模型评估是否足以解答用户疑问
5. **若满足**：进入下一步，组织最终回复
6. **若不满足**：触发重试逻辑，回到第 2 步继续调用工具或调整参数
7. **最终响应（Output）**：基于工具返回的数据，转化为自然语言输出给用户

如果工具返回的信息不充分，Agent 会自动回到第 2 步继续推理，直到获得满意的答案。整个过程对开发者是透明的。

**关键点**：原本大模型不具备查询天气的能力，所以无法回答天气问题。但当我们提供了 `get_weather` 工具以后，它就能自动调用工具获取实时数据并回答问题——这就是 Agent 的核心价值。
...
在大模型应用开发中，"模型"通常指**大语言模型 (Large Language Model, LLM)**。这类模型具备语言理解与生成能力，可执行文本生成、多语言翻译、信息摘要、复杂问答等任务，并演化出一系列高级功能。

从技术演进看，大模型经历了从**补全模型（Completion Model）**到**对话模型（Chat Model）**的转变。早期补全模型只能以类似"成语接龙"的方式补全文本，而现代对话模型具备更强的指令跟随能力，原生支持多轮对话、系统提示词、工具调用等功能。LangChain 通过统一的抽象层屏蔽了这些底层差异，让开发者专注于业务逻辑。
...
在 Agent 架构中，LLM 充当**核心推理引擎**，掌控着 Agent 的决策权：

- **工具选择**：根据用户意图，判定是否需要调用工具、调用哪个工具
- **参数生成**：从用户输入中提取关键信息，构造工具调用所需的参数
- **结果解读**：对工具返回的原始数据进行语境解析，判断是否满足需求
- **回复生成**：整合所有推理环节，将结构化数据转化为自然语言回复

简单来说，**LLM 是 Agent 的大脑，Tools 是 Agent 的手脚**。大脑负责思考和决策，手脚负责执行和获取信息。

### 4.3 LangChain 的模型集成优势

LangChain 对主流 LLM 实现了广泛适配，通过**标准化抽象层**提供一致的调用接口。开发者可以快速接入不同的模型供应商，在不同底层模型之间进行实验和切换。

目前 LangChain 已原生支持的模型供应商包括：OpenAI、Anthropic (Claude)、Google (Gemini)、DeepSeek、阿里云 (Qwen)、Cohere、Fireworks、Ollama (本地模型) 等。只需修改 `model` 参数即可在不同模型之间切换。

这种设计的核心价值在于**解耦**——当业务需要从一个模型迁移到另一个模型时，只需更换依赖包与模型名称，核心业务逻辑无需重构。

---

## 5. 模型初始化方式

LangChain 中实例化模型对象主要有两种路径：**自动化初始化**（通过 `init_chat_model` 工厂函数）和**手动实例化**（直接调用特定模型供应商的专用类）。

| 特性 | `init_chat_model` (工厂函数) | 手动实例化 (Model 类) |
|------|------|------|
| **便捷性** | 高，支持跨模型统一调用 | 一般，需导入特定类 |
| **灵活性** | 适用于通用场景 | 可访问特定模型类独有的高级属性 |
| **适用范围** | 仅限官方支持的常用模型 | 覆盖官方及社区所有已集成的模型 |
| **典型场景** | 快速启动、需要模型切换 | 需要精细控制、使用社区集成的模型 |

### 5.1 使用 `init_chat_model` 快速接入

如果只是想快速启动一个独立模型，推荐使用 `init_chat_model` 函数。只需指定供应商名称和模型名称，LangChain 会自动处理底层对象的创建——包括映射 API 地址、关联环境变量中的 API Key 等。
...
在 `.env` 文件中配置 API Key（环境变量名遵循 `{PROVIDER}_API_KEY` 命名规范）：
...
# 指定 model 参数后，LangChain 会自动映射 base_url 并关联 API Key
...
**核心价值**：需要从 DeepSeek 迁移到其他模型时，只需更换依赖包和 `model` 名称，业务逻辑无需改动：
...
`init_chat_model` 能自动识别主流供应商（如 OpenAI、DeepSeek、Anthropic 等）的预设配置，但对于不在原生支持列表中的模型（例如阿里云百炼的 **qwen-max**），需要通过自定义参数来建立连接。

由于 `init_chat_model` 无法自动推断其 API 地址和凭证，需要手动指定相关参数。好消息是许多国产模型兼容 OpenAI 协议，可以通过 `model_provider="openai"` 配合自定义 `base_url` 来接入。

**环境变量准备**：在 `.env` 文件中定义接入点与密钥：
...
除了基本的连接配置，`init_chat_model` 还支持透传模型运行参数，以控制生成行为：
...
`init_chat_model` 的底层逻辑本质上是调用各供应商对应的 **Model 类**。在 LangChain 生态中，除了官方核心库维护的模型类外，**社区驱动的集成 (langchain-community)** 提供了更广泛的模型支持。

当 `init_chat_model` 无法满足需求，或者你想直接使用社区提供的专用类时，可以手动实例化：
...
这种方式可以访问特定模型类独有的高级属性，适合需要精细控制模型行为的场景。

> 查看所有支持的模型集成：[LangChain Integrations](https://docs.langchain.com/oss/python/integrations/chat)

**小结**：实际开发中，推荐优先使用 `init_chat_model` 或 `create_agent` 的 `"provider:model"` 字符串格式。只有在需要访问特定模型类的独有属性时，才使用手动实例化。
