...
标准 ReAct 模式（行动-观察-思考...）是一步一步推进的——Agent 每次只决定下一步做什么。这种贪心决策在简单任务中表现良好，但在需要长期规划的复杂任务中会暴露严重缺陷。

| 问题 | 表现 | 具体例子 |
|------|------|----------|
| **缺乏全局视野** | 每步决策只看当前状态，无法预判后续依赖 | 先搜索了运动员的成绩，后续才发现需要先确认冠军是谁 |
...
graph TD
    ...
    Replanner -->|"还有未完成步骤<br/>或计划需要修改"| Executor
...
| **Replanner** | 评估进度，决定继续/修正/终止 | 原始输入 + 已完成步骤 + 剩余计划 | `Plan`（修订）/ `Response`（终止） | 强模型（GPT-4o / Claude Opus） |
...
状态是 Plan-and-Execute 系统的核心——所有组件通过读写共享状态来协作。下面的状态设计使用 `TypedDict` 配合 `operator.add` reducer，确保每步执行结果自动追加到历史中。
...
class PlanExecute(TypedDict):
    ...
    - past_steps: 已完成的 (步骤, 结果) 元组列表，使用 operator.add reducer，
...
执行器是一个标准的 ReAct Agent，但它在 Plan-and-Execute 架构中扮演单步执行的角色。它只负责执行当前步骤，不感知全局计划。
...
**关键理解**：Executor 内部的 ReAct 循环是"微观层面"的——它在单个步骤内部进行"思考-行动-观察"的循环。Plan-and-Execute 的外层循环是"宏观层面"的——它在步骤之间进行"执行-审视-调整"的循环。两层循环嵌套，各司其职。

### 3.4 创建规划器（Planner）

规划器是 Plan-and-Execute 系统的"大脑"。它使用 `with_structured_output` 确保 LLM 的输出严格符合 `Plan` Pydantic schema，避免自由文本解析的脆弱性。
...
class Plan(BaseModel):
    ...
    with_structured_output 会将 schema 注入 LLM system prompt，
...
# 1. 将 Plan JSON Schema 注入 LLM 的请求中
...
输出 Plan:
  steps: [
    ...
    "撰写完整的竞争格局分析报告，包含数据图表和结论"
  ]
```

### 3.5 创建重新规划器（Replanner）

Replanner 是 Plan-and-Execute 系统中最关键的组件——它是实现动态调整的核心。每执行完一步后，Replanner 根据实际结果决定：继续执行、修改计划还是终止并返回答案。
...
class Act(BaseModel):
    ...
    - 如果选择 Plan：表示还需要继续，输出修订后的步骤列表
    """
    action: Union[Response, Plan] = Field(
        description="如果要回应用户使用 Response，如果还需要执行步骤使用 Plan"
    )
...
```mermaid
graph TD
    START(["🚀 开始"]) -->|"收到执行结果"| Eval["📊 评估进度"]
    Eval -->|"对比 已完成 vs 目标"| Analyze{"🔍 分析差距"}

    Analyze -->|"仍有未完成的必要步骤"| Continue["📋 继续执行"]
    Analyze -->|"发现计划需要调整"| Replan["✏️ 修正计划"]
    Analyze -->|"已获得足够信息"| Finish["🏁 终止响应"]

    Continue -->|"移除已完成步骤"| OutPlan["📤 输出 Plan"]
    Replan -->|"插入 / 删除 / 重排序"| OutPlan
    Finish -->|"生成最终答案"| OutResp["💬 输出 Response"]

    OutPlan -->|"返回 Executor 继续"| END1(["🔄 循环执行"])
    OutResp -->|"结束图执行"| END2(["✅ 结束"])

    style START fill:#d5f5e3,stroke:#27ae60,stroke-width:2px
    style Eval fill:#d6eaf8,stroke:#2980b9,stroke-width:2px
    style Analyze fill:#fdebd0,stroke:#e67e22,stroke-width:2px
    style Continue fill:#d6eaf8,stroke:#2980b9,stroke-width:2px
    style Replan fill:#fdebd0,stroke:#e67e22,stroke-width:2px
    style Finish fill:#fadbd8,stroke:#e74c3c,stroke-width:2px
    style OutPlan fill:#e8daef,stroke:#8e44ad,stroke-width:2px
    style OutResp fill:#e8daef,stroke:#8e44ad,stroke-width:2px
    style END1 fill:#d5f5e3,stroke:#27ae60,stroke-width:2px
    style END2 fill:#d5f5e3,stroke:#27ae60,stroke-width:2px
```
...
async def execute_step(state: PlanExecute) -> dict:
    ...
    关键设计：
    - 只执行 plan[0]（当前第一步），非整个计划
    - 将完整计划作为上下文传给执行器，帮助它理解全局
...
def should_end(state: PlanExecute) -> Literal["agent", "__end__"]:
    ...
    逻辑：如果 Replanner 已经输出 response（非空）
...
workflow.add_edge("agent", "replan")   # 执行 → 重规划器
...
inputs = {
    "input": "2024年巴黎奥运会100米自由泳决赛冠军的家乡是哪里？请用中文回答"
}
...
[replan]:
  {'plan': ['查找潘展乐的家乡']}
  # Replanner 将步骤中的"该冠军"替换为具体名字"潘展乐"
...
为了更直观地展示 Plan-and-Execute 的动态调整能力，以下用一个市场调研案例贯穿全流程
...
sequenceDiagram
    ...
    P->>E: 步骤1: 搜索市场整体数据
    E-->>R: 结果: "2024年销量突破1000万辆，渗透率40%"

    R->>R: 评估: 数据获取成功，继续
    R->>E: 步骤2: 搜索主要厂商份额

    E-->>R: 结果: "比亚迪35%, 特斯拉8%, 吉利7%, 理想6%..."
    Note over R: 发现: 新信息——吉利份额显著提升，原计划未包含吉利的技术路线
...
| 准则 | 说明 | 反模式 |
|------|------|--------|
| **规划粒度适中** | 每个步骤应该是一个可独立执行的任务，不要太细（原子级）也不要太粗（解决所有问题） | 步骤过于笼统导致执行器无法操作 |
| **模型分工** | Planner 和 Replanner 使用强模型（GPT-4o），Executor 可用弱模型（GPT-4o-mini） | 所有组件使用同一个模型，浪费成本 |
...
| **错误感知** | 执行失败时将错误信息写入 past_steps，让 Replanner 能感知并调整 | 吞掉错误，Replanner 以为步骤已完成 |
| **计划可追踪** | 记录每次重规划前后的计划变化，便于调试和审计 | 只保留最新计划，无法回溯调整历史 |

---

## 总结

Plan-and-Execute 模式通过"先规划再执行、边执行边调整"的策略，解决了复杂多步骤任务中的长期规划问题。LangGraph 的图结构和条件边机制天然支持这种模式的实现：
