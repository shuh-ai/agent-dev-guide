...
在构建基于大语言模型（LLM）的应用时，开发者面临一个核心挑战：**如何让模型不仅能回答问题，还能执行多步骤任务、调用外部工具并在必要时寻求人工协助**。

传统 LangChain Chain 采用线性管道模式，输入经过一系列预定义的处理步骤后输出。这种模式在简单的问答场景下工作良好，但在以下场景中显得力不从心：
...
**LangGraph** 正是为解决这些问题而生的底层图框架。它受 Google Pregel 和 Apache Beam 的启发，公共接口借鉴 NetworkX，专为构建**有状态、多角色、带循环**的 LLM 应用而设计。

### 1.2 核心优势对比

| 特性 | LangChain Chain | LangGraph |
|------|----------------|-----------|
| **执行模式** | 线性 DAG，单向无环 | 图结构，**支持循环** |
| **状态管理** | 隐式传递，难以干预 | 显式 State 定义，**每个节点可读写** |
| **持久化** | 无内置机制 | **Checkpointer** 断点续传 |
| **人机交互** | 不支持 | **Interrupt** 挂起恢复 |
| **并行执行** | 需手动实现 | **Send API** 原生支持 |
...
LangGraph 的底层图算法基于**消息传递（Message Passing）**机制，灵感来自 Google Pregel 系统。程序以离散超级步骤（Super Step）运行。

- **超级步骤**：图节点上的每次迭代，并行运行的节点属于同一个超级步骤，顺序运行的节点属于不同超级步骤
...
State 是图中所有节点共享的数据结构。每个图执行都有一个 State 实例，节点读取它并在执行后返回更新。State 可以是任意 Python 类型，但通常使用 `TypedDict` 或 Pydantic `BaseModel`。
...
- 未指定 Reducer 时，默认行为是**覆盖**（后写入的替换前一个）
...
- **入口边**：用户输入到达时首先调用的节点（`add_edge(START, "first_node")`）
- **条件入口边**：根据自定义逻辑从不同的节点开始（`add_conditional_edges(START, routing_func)`）

---

## 三、快速起步：第一个 ReAct Agent

下面是一个完整的搜索 Agent 示例，演示了 LangGraph 的核心开发流程：**定义状态、添加节点、连接边、编译、执行**
...
`bind_tools()` 方法将工具的 JSON Schema 注入 LLM 的系统提示中，使模型能够
- 理解每个工具的功能和参数
- 在推理过程中决定是否需要调用工具
- 生成符合工具参数规范的调用请求
...
- `add_edge`：添加普通边（Tools 到 Agent 形成循环）
...
当需要**默认值**、**数据验证**、**序列化控制**时，可以使用 Pydantic BaseModel。
...
| 接口 | 作用 | 必要性 |
|------|------|--------|
| `StateGraph(state_schema)` | 初始化图，声明状态结构 | 必须 |
| `add_node(name, function)` | 注册节点 | 必须 |
| `add_edge(source, target)` | 添加普通边 | 按需 |
| `add_conditional_edges(source, router)` | 添加条件边 | 按需 |
| `set_entry_point(node)` | 设置入口节点 | 必须 |
| `compile(checkpointer, interrupt_before)` | 编译图，返回 Runnable | 必须 |
| `invoke(inputs, config)` | 同步执行 | 执行时 |
| `ainvoke(inputs, config)` | 异步执行 | 异步执行时 |
| `stream(inputs, config, stream_mode)` | 流式执行 | 流式执行时 |
...
在掌握了 LangGraph 的基本构建流程后，下一篇将深入探讨
- **State（状态）** Schema 定义与 Reducer 归约器机制
- **Node（节点）** 的同异步实现与特殊节点
