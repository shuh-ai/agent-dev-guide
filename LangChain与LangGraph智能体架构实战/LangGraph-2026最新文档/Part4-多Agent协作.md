...
> **适用场景**：数据采集与可视化、报告生成、多步推理链、复杂业务流程自动化
...
单个 Agent 在工具数量较少时可以高效运行，但面对复杂任务时，集中式 Agent 会遇到以下瓶颈：

| 问题 | 表现 | 根因 |
|------|------|------|
| **上下文污染** | 大量工具描述挤占提示词空间，干扰推理 | 所有工具共享同一上下文窗口 |
| **工具选择退化** | 工具超过 10 个时，LLM 选择正确工具的概率显著下降 | 选择空间指数增长 |
| **职责耦合** | 一个系统提示既要搜索又要绘图，角色模糊 | 混合职责导致行为不可预测 |
| **错误传播** | 中间步骤出错，后续步骤无法感知并修正 | 缺乏检查点和反馈机制 |

### 1.2 多 Agent 的"分而治之"策略

**多 Agent 系统**将复杂任务拆解为多个专业化的子任务，每个 Agent 只负责一个领域：

- 每个 Agent 只拥有完成其职责所需的最小工具集
- Agent 之间通过图结构（Graph）进行确定性的任务传递
...
在 LangGraph 中，多 Agent 系统有多种拓扑结构可选。选择哪种取决于任务特性：
...
graph LR
    ...
    subgraph 层级委托
        H1[Supervisor] -->|子任务 1| H2[Agent A]
        H1 -->|子任务 2| H3[Agent B]
        H2 -->|结果| H1
        H3 -->|结果| H1
    end
...
| 模式 | 适用场景 | 优点 | 缺点 |
|------|----------|------|------|
| **顺序链** | 流水线式处理（如：搜索 → 分析 → 报告） | 简单可控 | 不灵活，无法回退 |
| **路由分发** | 任务类型明确，各分支独立 | 并行效率高 | 路由逻辑需精心设计 |
| **层级委托** | 复杂任务需动态分解 | 灵活，支持递归 | 实现复杂度高 |
...
sequenceDiagram
    ...
    S->>R: sender 路由回 Researcher
    R->>S: 整理数据, 传递给 Chart Generator
    end
    ...
    S->>C: sender 路由回 Chart Generator
    C->>S: FINAL ANSWER
    end
...
以下是基于 LangGraph 最新 API 的完整实现。代码包含详细的中文注释，标注了每个设计决策的原因。
...
class AgentState(TypedDict):
    ...
    - messages: 使用 operator.add 做 reducer，新消息自动追加而非覆盖
...
def python_repl(code: str) -> str:
    """执行 Python 代码以生成图表或处理数据。需要 print() 输出结果。"""
    try:
...
def create_agent(llm, tools_list, system_message: str):
    ...
    prompt = ChatPromptTemplate.from_messages([
        (
            ...
            "使用提供的工具推进问题解答。"
            "如果你不能完全回答，没关系，另一个拥有不同工具的助手会接着处理。"
            "如果你或其他助手得到了最终答案或交付物，"
...
# 研究员 Agent：专注于数据搜索与整理
research_agent = create_agent(
    llm,
    [tavily_tool],
    system_message="你应该提供准确的数据供 chart_generator 使用。搜索结果需要包含具体数字。",
)
...
graph LR
    subgraph "执行流程示意"
        ...
        S2 --> S3["3️⃣ Researcher<br/>整理数据,传递给 Chart"]
...
**为什么不用更复杂的终止检测？** 因为 FINAL ANSWER 是一个显式的、由 LLM 主动发出的信号。相比隐式检测（如"没有工具调用就结束"），它更可靠——Agent 可能在没有工具调用时仍需将结果传递给下一个 Agent。

### 3.3 Agent 工厂模式

`create_agent` 工厂函数实现了 Agent 的标准化创建，将"系统提示 + 工具绑定 + 模型调用"封装为一个可复用的构建单元：
...
工厂模式的优势：
- **一致性**：所有 Agent 共享相同的协作协议（FINAL ANSWER）
- **可维护性**：修改系统提示模板只需改一处
- **可扩展性**：新增 Agent 只需一行调用

### 3.4 条件边与状态 Reducer

LangGraph 的 `AgentState` 使用 `Annotated[Sequence[BaseMessage], operator.add]` 定义 `messages` 字段。这里的 `operator.add` 是一个 **reducer 函数**：
...
## 四、现代范式：带 Supervisor 的多 Agent 系统
...
Supervisor 模式下，每个 Agent 不再需要知道"下一个 Agent 是谁"——这个决策完全由 Supervisor 统一做出。
...
# ==========================================
# 2. 创建专业化 Agent（使用 create_react_agent）
# ==========================================

# create_react_agent 是 LangGraph 内置的 ReAct Agent 工厂
# 它自动处理"思考 → 工具调用 → 观察"的循环
...
**隔离状态**（高级模式）：
- 每个 Agent 有自己的私有状态，通过显式的 "handoff" 传递必要信息
- 优点：上下文干净，安全性更好
- 缺点：需要精心设计交接协议
...
多 Agent 系统的核心优势是可扩展性。添加一个新的 Math Agent 只需四步：
...
| **Sender 追踪** | 始终追踪消息发送者，确保工具结果回到正确的 Agent | 假设工具结果总是回到同一个 Agent |
...
多 Agent 系统通过"分而治之"的策略，将复杂任务拆解为多个专业 Agent 协作完成。在 LangGraph 中实现多 Agent 协作的关键要素：
