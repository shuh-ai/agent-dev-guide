...
## 🏗️ 支柱二：机械化架构约束（Mechanized Architectural Constraints）
...
> 🏛️ **核心原则：CLAUDE.md 是建议，Hooks 是法律。** 建议可以被忽略，法律不行。
...
> ⚠️ **关键细节**：退出码 `2` 是拦截的"魔法数字"——不是 `1`，是 `2`。这个设计精确区分了"Hook 脚本自己出错"（exit 1）和"Hook 有意拦截违规操作"（exit 2）。
...
> ⚙️ **"品味捕获一次，强制执行无限次。"** 你只需要把对代码质量的判断标准编码成规则一次，Agent 就会持续不断地按这个标准执行检查。
...
> 🗑️ **Harness 必须设计成可删除的。** 因为今天需要复杂管线才能搞定的活，明天可能一个提示词就搞定了。
...
2. 🗑️ **Build to Delete**：模块化架构，随时准备替换或删掉任意组件
