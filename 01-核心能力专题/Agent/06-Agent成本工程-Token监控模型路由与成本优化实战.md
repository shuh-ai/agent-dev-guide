...
| 模型 | 输入价格 | 输出价格 | 适用场景 |
|------|----------|----------|----------|
| **DeepSeek V4 Pro** | ¥12.5 | ¥25.0 | 高端推理、复杂任务 |
| **DeepSeek V4 Flash** | ¥1.0 | ¥2.0 | 通用任务、高性价比 |
| **Kimi K2.6** | ¥6.8 | ¥29.0 | Agent、代码生成、长文本 |
| **Kimi K2.5** | ¥4.3 | ¥21.6 | 多模态、成本敏感 |
| **千问3.7 Max** | ¥2.0 | ¥8.0 | 通用任务、阿里生态 |
| **GLM-5** | ¥2.0 | ¥8.0 | 通用任务、中文优化 |
| **MiniMax abab7** | ¥2.0 | ¥8.0 | 通用任务、多模态 |

> **价格说明：** 
> - 国际模型价格来自 OpenAI 和 Anthropic 官方定价页面（2026年6月）
> - 国内模型价格来自各平台官方定价（2026年6月）
> - DeepSeek V4 于 2026 年 4 月发布，V4-Flash 定位为低成本生产方案
> - Kimi K2.6 于 2026 年 7 月发布，支持 262K 上下文，支持自动缓存（缓存价格 $0.15-0.16/M）
> - DeepSeek 和 Kimi 均支持缓存命中，实际成本可降低 80%
> - 汇率：1 USD ≈ 7.2 CNY
...
2. **国内模型价差 12 倍**：DeepSeek V4 Pro (¥12.5) vs DeepSeek V4 Flash (¥1.0)
3. **跨境价差 36 倍**：GPT-5.5 ($5.00) vs DeepSeek V4 Flash ($0.14)
4. **DeepSeek V4 Flash 性价比最高**：同级别能力下，价格仅为国际模型的 1/10~1/36
5. **Kimi K2.6 定位高端**：价格接近国际模型，但支持 262K 长上下文和自动缓存
6. **缓存可大幅降低成本**：DeepSeek 和 Kimi 均支持自动缓存，缓存命中后成本降低 80%
...
class TokenMonitor:
    ...
    def calculate_cost(self, model: str, input_tokens: int, output_tokens: int) -> float:
        ...
        pricing = {
            ...
            # 国内模型（人民币/百万token，需除以7.2转美元）
            "deepseek-v4-flash": {"input": 1.0/7.2, "output": 2.0/7.2},
            "deepseek-v4-pro": {"input": 12.5/7.2, "output": 25.0/7.2},
            "kimi-k2.6": {"input": 6.8/7.2, "output": 29.0/7.2},
            "kimi-k2.5": {"input": 4.3/7.2, "output": 21.6/7.2},
            "qwen3.7-max": {"input": 2.0/7.2, "output": 8.0/7.2},
            "glm-5": {"input": 2.0/7.2, "output": 8.0/7.2},
            "minimax-abab7": {"input": 2.0/7.2, "output": 8.0/7.2},
        }
