...
```
┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌──────────┐
│  源文档   │ → │ 文档切分  │ → │ 向量化   │ → │ 向量检索  │ → │ LLM 生成 │
│  PDF/MD  │    │ Chunking │    │Embedding│    │ Top-K   │    │ 最终回答  │
└──────────┘    └──────────┘    └──────────┘    └──────────┘    └──────────┘
```
...
```
用户 Query
    ├─→ 稠密检索（Dense Embedding）  → Top-20
    ├─→ 稀疏检索（BM25）            → Top-20    ──→ Reranker → Top-5 → LLM
    ├─→ 关键词实体检索（Elasticsearch）→ Top-10
    └─→ 最近对话上下文检索           → Top-5
```
...
│                      检索层                                      │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────────┐   │
│  │ 稠密检索  │  │ 稀疏检索  │  │ 元数据过滤 │  │ 知识图谱查询  │   │
│  │ (Dense)  │  │ (BM25)   │  │ (Filter)  │  │ (Cypher)    │   │
│  └──────────┘  └──────────┘  └──────────┘  └──────────────┘   │
│                         │                                       │
│                    ┌─────▼──────┐                               │
│                    │  Reranker  │  ← 精排 Top-K                  │
...
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────────────┐  │
│  │  向量数据库   │  │   图数据库    │  │   全文检索引擎        │  │
│  │ Milvus/Qdrant│  │   Neo4j      │  │   Elasticsearch      │  │
│  └──────────────┘  └──────────────┘  └──────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
                              │
                              ▼
...
│                      数据处理层                                  │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────────┐   │
│  │ 文档解析  │  │ 智能切分  │  │ Embedding│  │  多模态处理   │   │
│  │ MinerU等 │  │ Chunking │  │  BGE-M3  │  │  OCR + VLM   │   │
│  └──────────┘  └──────────┘  └──────────┘  └──────────────┘   │
└─────────────────────────────────────────────────────────────────┘
```
...
1. **从零搭建企业知识库问答系统**：完整项目实战，含代码
2. **Chunking 深度解析**：分块策略对比实验与最佳实践
3. **向量数据库实战**：Milvus vs Qdrant 性能对比与选型
4. **GraphRAG 入门到实战**：知识图谱构建与查询
5. **Agentic RAG 开发指南**：LangGraph 实现自主检索 Agent
6. **多模态 RAG 实战**：PDF 图文混排的完整检索方案
