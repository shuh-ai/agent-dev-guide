# Part 4. GraphRAG 快速入门与原理详解

> 本文是 AI 大模型技术社区「RAG 检索增强生成」系列的第 4 篇，深入讲解微软 GraphRAG 的原理、索引构建六步流程和查询引擎机制，附带从一段原始文本到完整知识图谱的全流程示例。
>
> 前置阅读：[Part 1. RAG 技术体系全景](../RAG/Part1-RAG技术体系全景.md)
...
│                      索引阶段 (Indexing)                         │
│                                                                 │
│  原始文档 → 文本切分 → 实体/关系抽取 → 社区聚类 → 图谱存储       │
│                                                                 │
│  输出: Parquet 文件（实体表、关系表、社区报告表、文本单元表）       │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                      查询阶段 (Query)                             │
│                                                                 │
│  用户提问 → 局部搜索(实体+关系+文本+社区报告) → LLM 生成回答       │
│                                                                 │
│  输出: 基于图谱上下文的增强回答                                    │
└─────────────────────────────────────────────────────────────────┘
```

![GraphRAG 架构全景](https://graphrag.com/_astro/graphrag-diagram.gzIXlJ0V_Z1168rD.svg)
...
![GraphRAG 索引管线](https://dist.neo4j.com/wp-content/uploads/20240813110226/indexing-pipeline-graphrag.webp)

> ▲ GraphRAG 索引管线全景：文档 → 文本切分 → 实体/关系提取 → 社区检测 → 社区摘要 → 嵌入存储 → Parquet 输出（来源：Neo4j）
...
- **Part 5**：GraphRAG 实战——用 LangChain + Neo4j 搭建可查询的知识图谱问答系统
