...
> 本文是 AI 大模型技术社区「RAG 检索增强生成」系列的第 3 篇，深入介绍两款 PDF → Markdown 核心工具：Allen AI 的 **olmOCR**（VLM 型 OCR，SOTA 级精度）和阿里 OpenDataLab 的 **MinerU**（一站式文档解析管线）。
>
...
> ▲ olmOCR 系统架构：基于视觉语言模型（VLM），单次前向即可将 PDF 页转为结构化 Markdown（来源：Allen AI 官方）
...
> ▲ olmOCR-Bench 综合评测：olmOCR 2（7B 参数）在自然阅读顺序、复杂表格、公式识别等多个维度上达到 SOTA（来源：Unsiloed AI）
...
> ▲ MinerU 文档解析 Pipeline：PDF 输入 → 布局分析 → OCR 识别 → 结构重建 → Markdown/JSON 输出（来源：OpenDataLab）
...
- **Part 4**：多模态 RAG 项目开发实战（完整项目 + 前端部署）
- **Part 5**：RAG 系统评估与优化（RAGAS + Reranker 实战）
