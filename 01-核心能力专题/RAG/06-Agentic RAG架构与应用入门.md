...
> 本文是 AI 大模型技术社区「RAG 检索增强生成」系列的第 5 篇。
>
...
> ▲ RAG 高层架构：用户提问 → 检索增强 → LLM 生成回答（来源：Microsoft Azure Architecture Center）
...
> ▲ ReAct 代理的 Thought → Action → Observation 循环（来源：Medium）
...
> ▲ LangGraph 有向图工作流：节点代表处理步骤，边代表流转关系（来源：IBM）
...
> ▲ LangGraph 构建的 ReAct Agent 图结构：agent ↔ tools 循环（来源：Dylan Castillo）
...
> ▲ Agentic RAG 架构：Agent 作为编排中心，协调多个检索工具和数据源（来源：Deepchecks）
...
- **Part 6**：RAG 系统评估与优化（RAGAS + Reranker 实战）
- **Part 7**：多 Agent 协作——CrewAI 实战
- **Part 8**：Self-RAG 与 Adaptive RAG 深度解析
